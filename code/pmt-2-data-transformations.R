...
saveRDS(under_sampled_data, "data/out/data_under_sampled.rds")
saveRDS(over_sampled_data, "data/out/data_over_sampled.rds")
...
balanced_df$poor_20 <- as.factor(balanced_df$class)
balanced_df$class <- NULL

saveRDS(balanced_df, "data/out/data_smote.rds")

#tables of each class and sampling technique
table(original_data$poor_20)
table(over_sampled_data$poor_20)
table(under_sampled_data$poor_20)
table(balanced_df$poor_20)

## PCA ---------------
data1 <- select(data, -poor_20)

# Step 1: PCA Only
# ----------------
pca_result <- prcomp(data1, center = TRUE, scale. = TRUE)
# Create a new dataframe from PCA scores
pca_data <- data.frame(pca_result$x)
pca_data$poor_20 <- data$poor_20
saveRDS(pca_data, "data/out/pca_data.rds")

# Step 2: Whitening Only
# ----------------------
# Apply the whitening transformation directly
whitened_scores <- sweep(pca_result$x, 2, pca_result$sdev, "/")
whitened_data <- data.frame(whitened_scores)
whitened_data$poor_20 <- data$poor_20
saveRDS(whitened_data, "data/out/whitened_data.rds")
